...
AbstractComponent *ComponentsFactory::create(const QString &component_name,
  ...
  } else if (component_name == "SignalSource") {
    new_component = new SignalSourceComponent(parent);
  } else if (component_name == "SignalSource8") {
    new_component = new SignalSource8Component(parent);
  } else {
