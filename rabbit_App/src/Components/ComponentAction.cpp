...
ComponentAction *ComponentActionFactory::create(const QString &text,
  ...
  } else if (text == "SignalSource") {
    action->setIcon(SignalSourceComponent::componentIcon());
  } else if (text == "SignalSource8") {
    action->setIcon(SignalSource8Component::componentIcon());
  } else {
